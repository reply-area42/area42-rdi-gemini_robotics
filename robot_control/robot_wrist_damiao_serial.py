...
from unitree_sdk2py.idl.default import unitree_go_msg_dds__MotorCmd_,unitree_go_msg_dds__MotorState_  # idl
...
class Damiao_Wrist_Controller_Serial:
    def __init__(self,interface, left_hand_array, right_hand_array, dual_hand_data_lock = None, dual_hand_state_array = None,
                       dual_hand_action_array = None, fps = 100.0, Unit_Test = False, simulation_mode = False):
        logger_mp.info("Initialize Damiao_Wrist_Controller_Serial...")
        self.fps = fps
        ...
        if self.simulation_mode:
            ...
            ChannelFactoryInitialize(0, interface) # Change to your network interface name
 
        # Publisher/subscriber unificati
        ...
        # Shared Arrays for hand states (12 motori: 6 destra + 6 sinistra)
        self.hand_state_array = Array('d', 12, lock=True)
        ...
        logger_mp.info("Initialize Inspire_Controller_DFX_Serial OK!\n")
 
    def _subscribe_hand_state(self):
        while True:
            hand_msg = self.handstate_subscriber.Read()
            if hand_msg is not None:
                with self.hand_state_array.get_lock():
                    for i in range(12):
                        self.hand_state_array[i] = hand_msg.states[i].q
            time.sleep(0.002)
    ...
    def ctrl_dual_hand(self, left_q_target, right_q_target):
        ...
        """
        cmd = MotorCmds_()
        cmd.cmds = [unitree_go_msg_dds__MotorCmd_() for _ in range(12)]
        for i in range(6):
            cmd.cmds[i].q = float(right_q_target[i])
            cmd.cmds[i+6].q = float(left_q_target[i])
        self.handcmd_publisher.Write(cmd)
        # print(cmd)
