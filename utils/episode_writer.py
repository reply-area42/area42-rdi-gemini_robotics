...
class EpisodeWriter():
    ...
    def add_item(self, colors, depths=None, states=None, actions=None, tactiles=None, audios=None, sim_state=None, ext_colors=None, ext_depths=None):
    ...
    def _process_item_data(self, item_data):
        ...
        if depths:
            for idx_depth, (depth_key, depth) in enumerate(depths.items()):
                ...
                if not cv2.imwrite(os.path.join(self.depth_dir, depth_name), depth.astype(np.uint16)):
